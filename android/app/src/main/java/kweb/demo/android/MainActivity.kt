...
import kweb.*
import kweb.demos.todo.ToDoState
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        TodoApp(this.baseContext)

        val webView: WebView = findViewById(R.id.webViewWidget)

        webView.settings.javaScriptEnabled = true
        webView.webViewClient = WebViewClient()
        webView.loadUrl("http://localhost:7659")
    }

    class TodoApp (context:Context){

        private val logger = KotlinLogging.logger {}

        val state = ToDoState(context.filesDir.toPath())
        ...
        init {
            ...
            server = Kweb(port = 7659, debug = true, plugins = plugins, buildPage = {
                doc.head.new {
                    ...
                    meta(
                        name = "Description",
                        content = "A simple To Do list app to demonstrate Kweb"
                    )
                }
                                ...
                                path("/lists/{id}") { params ->
                                    ...
                                    render(params.getValue("id")) { listId ->
                                        ...
                                        try {
                                            ...
                                            val list: KVar<ToDoState.List> =
                                                toVar(state.lists, listId)
        ...
        private fun handleAddItem(input: InputElement, list: KVar<ToDoState.List>) {
            GlobalScope.launch {
                ...
                input.setValue("")
                val newItem =
                    ToDoState.Item(generateNewUid(), Instant.now(), list.value.uid, newItemText)
